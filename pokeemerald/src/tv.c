...
static void DoTVShowDipartimentoTrendWatcherNetwork(void);
...
static const u8 *const sTVDipartimentoTrendWatcherNetworkTextGroup[] = {
...
{
    // Check if not in Rimini house map group
    if (gSaveBlock1Ptr->location.mapGroup != MAP_GROUP(RIMINI_TOWN_BRENDANS_HOUSE_1F))
        return PLAYERS_HOUSE_TV_NONE;

    // Check if not in player's house (dependent on gender)
    if (gSaveBlock2Ptr->playerGender == MALE)
    {
        if (gSaveBlock1Ptr->location.mapNum != MAP_NUM(RIMINI_TOWN_BRENDANS_HOUSE_1F))
            return PLAYERS_HOUSE_TV_NONE;
    }
    else
    {
        if (gSaveBlock1Ptr->location.mapNum != MAP_NUM(RIMINI_TOWN_MAYS_HOUSE_1F))
            return PLAYERS_HOUSE_TV_NONE;
    }
...
void DoTVShow(void)
{
    if (gSaveBlock1Ptr->tvShows[gSpecialVar_0x8004].common.active)
    {
        ...
        case TVSHOW_TODAYS_RIVAL_TRAINER:
            ...
            break;
        case TVSHOW_TREND_WATCHER:
            DoTVShowDipartimentoTrendWatcherNetwork();
            break;
        case TVSHOW_TREASURE_INVESTIGATORS:
...
static void DoTVShowDipartimentoTrendWatcherNetwork(void)
{
    ...
    ShowFieldMessage(sTVDipartimentoTrendWatcherNetworkTextGroup[state]);
}
