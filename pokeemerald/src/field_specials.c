...
{
    if (gLastUsedWarp.mapNum == MAP_NUM(EMILIA_ROAD_PISTA_SKATE_SOUTH) && gLastUsedWarp.mapGroup == MAP_GROUP(EMILIA_ROAD_PISTA_SKATE_SOUTH))
        return;
...
static void PCTurnOnEffect_SetMetatile(s16 isScreenOn, s8 dx, s8 dy)
{
    ...
    if (isScreenOn)
    {
        ...
        if (gSpecialVar_0x8004 == PC_LOCATION_OTHER)
            ...
            metatileId = METATILE_CasaGasSte_BrendanPC_Off;
        else if (gSpecialVar_0x8004 == PC_LOCATION_MAYS_HOUSE)
            metatileId = METATILE_CasaGasSte_MayPC_Off;
    }
    else
    {
        ...
        if (gSpecialVar_0x8004 == PC_LOCATION_OTHER)
            ...
            metatileId = METATILE_CasaGasSte_BrendanPC_On;
        else if (gSpecialVar_0x8004 == PC_LOCATION_MAYS_HOUSE)
            metatileId = METATILE_CasaGasSte_MayPC_On;
    }
...
bool8 UsedPokemonCenterWarp(void)
{
    static const u16 sPokemonCenters[] =
    {
        ...
        MAP_RIMINI_MARE_POKEMON_CENTER,
...
u32 GetMartEmployeeObjectEventId(void)
{
    ...
    {
        { MAP_GROUP(BOLOGNA_TOWN_MART),     MAP_NUM(BOLOGNA_TOWN_MART),     LOCALID_BOLOGNA_MART_CLERK },
        { MAP_GROUP(RIMINI_MARE_MART), MAP_NUM(RIMINI_MARE_MART), LOCALID_RIMINIMARE_MART_CLERK },
...
// Defeating Kamencik or participating in a Contest increments the counter by 2
...
u8 TryGainNewFanFromCounter(u8 incrementId)
{
    ...
    {
        [FANCOUNTER_DEFEATED_KAMENCIK]    = 2,
...
static void BufferFanClubTrainerName_(struct LinkBattleRecords *linkRecords, u8 whichLinkTrainer, u8 whichNPCTrainer)
{
    ...
    if (record->name[0] == EOS)
    {
        ...
        case 3:
            ...
            break;
        case 4:
            StringCopy(gStringVar1, gText_Fraboni);
            break;
        case 5:
            StringCopy(gStringVar1, gText_Castellani);
            break;
        default:
            StringCopy(gStringVar1, gText_Wallace);
            break;
